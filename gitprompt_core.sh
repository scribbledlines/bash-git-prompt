...
function updatePrompt() {
  ...
  # for i in "${!git_status_fields[@]}"; do
  #     echo "${i}] ${git_status_fields[${i}]}"
  # done
  ...
  if [[ "." == "${GIT_REMOTE_USERNAME_REPO}" ]]; then
      GIT_REMOTE_USERNAME_REPO="${git_status_fields[10]}"
  fi
  ...
  GIT_PS1="${NEW_PROMPT//_LAST_COMMAND_INDICATOR_/${LAST_COMMAND_INDICATOR}${ResetColor}}"
